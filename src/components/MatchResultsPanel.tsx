...
export default function MatchResultsPanel({ setActiveTab, activeTab }: { setActiveTab: (tab: any) => void, activeTab?: string }) {
 ...
 className="border border-[#c9a84c]/30 flex flex-col overflow-hidden transition-all bg-portal-surface shadow-[0_4px_20px_rgba(201,168,76,0.08)] relative"
 >
 {/* Glow accent */}
 <div className="absolute top-0 right-0 w-28 h-28 bg-[#c9a84c]/10 blur-2xl -mr-8 -mt-8 pointer-events-none" />

 {/* Header */}
 <header className="p-3 border-b flex items-center justify-between border-[#c9a84c]/20 bg-[#c9a84c]/5 relative z-10">
 <h3 className="text-[13px] font-normal flex items-center text-portal-text-main gap-1.5">
 <Trophy size={12} className="text-[#c9a84c]" />
 <span>FIFA WC 2026</span>
 <span className="text-[10px] bg-[#c9a84c]/20 text-[#c9a84c] px-1.5 py-0.5 uppercase tracking-widest font-normal ring-1 ring-[#c9a84c]/30 ml-1">
 {isAllScores ? 'Schedule' : 'Results'}
 </span>
 {apiStatus === 'live' && (
 <span className="ml-auto flex items-center gap-1 text-[8px] text-green-500">
 <span className="w-1.5 h-1.5 bg-green-500 animate-pulse"></span>Live
 </span>
 )}
 {apiStatus === 'fallback' && (
 <span className="ml-auto flex items-center gap-1 text-[8px] text-amber-500">
 ...
 className="flex items-center gap-1 text-[9px] text-portal-text-muted hover:text-[#c9a84c] transition-colors disabled:opacity-50 px-1.5 py-1 hover:bg-[#c9a84c]/10"
 >
 ...
 <div
 key={idx}
 className="flex flex-col p-2 bg-portal-bg border border-portal-border/60 hover:border-[#c9a84c]/30 hover:shadow-[0_0_10px_rgba(201,168,76,0.06)] transition-all"
 >
 ...
 alt={m.team1}
 className="w-6 h-6 object-cover border border-portal-border/50 mb-0.5"
 onError={e => { (e.target as HTMLImageElement).style.opacity = '0.3'; }}
 />
 <span className="text-[11px] text-portal-text-main truncate w-full text-center">
 {m.team1}
 </span>
 </div>
 ...
 <div className="text-sm text-portal-text-main tracking-widest">
 {m.score1} <span className="text-portal-text-muted/40">-</span> {m.score2}
 </div>
 <div className="text-[10px] uppercase tracking-widest text-[#c9a84c] bg-[#c9a84c]/10 px-1.5 py-0.5 mt-0.5">
 {m.status}
 </div>
 <div className="text-[10px] text-portal-text-muted mt-0.5">{m.date}</div>
 </div>
 ...
 <div className="mt-1.5 text-[10px] text-portal-text-muted border-t border-portal-border/40 pt-1 text-center truncate px-1">
 ...
 <span className="text-[10px] font-normal text-portal-text-muted flex items-center gap-1">
 <Clock size={9} />
 Updated {lastUpdated.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
 </span>
 <span className="text-[10px] font-normal text-[#c9a84c]">
 Next: {formatCountdown(countdown)}
 </span>
 </div>
 <button
 onClick={() => setActiveTab(isAllScores ? 'fifa' : 'fifaAllScores')}
 className="w-full text-center text-[11px] font-normal text-[#c9a84c] hover:bg-[#c9a84c]/10 py-2 transition-colors"
 >
