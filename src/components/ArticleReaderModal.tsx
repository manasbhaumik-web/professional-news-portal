  ...
  return (
    <div id="article-reader-root-modal" className="fixed inset-0 bg-portal-bg bg-opacity-95 z-50 flex flex-col overflow-y-auto antialiased">
      ...
      <div className={`flex-1 transition-all duration-300 ${isCleanMode
        ...
        <div className={`max-w-4xl mx-auto px-4 sm:px-6 py-10 sm:py-16 ${isCleanMode ? 'pb-32' : 'pb-20'}`}>
          {isCleanMode && (
          ...
          <h1 className={`text-2xl sm:text-4xl lg:text-5xl font-bold tracking-tight font-serif mb-6 leading-tight ${isCleanMode ? 'text-inherit' : 'text-portal-text-main'}`}>
            {selectedArticle.title}
          </h1>

          <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-6 mb-8 text-xs font-mono ${isCleanMode ? 'border-zinc-800 text-zinc-400' : 'border-portal-border text-portal-text-muted'}`}>
          ...
          {/* Fact Check & Bias Radar Block */}
          <div className={`mb-8 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center p-4 rounded-xl border ${isCleanMode ? 'bg-zinc-900 border-zinc-800' : 'bg-emerald-50/50 border-emerald-500/20 shadow-sm'}`}>
            <div className="flex items-center gap-4">
              <div className={`flex items-center justify-center w-12 h-12 rounded-full border-4 ${isCleanMode ? 'border-cyan-900 text-cyan-400' : 'border-emerald-500/30 text-emerald-600'} relative`}>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 border ${isCleanMode ? 'bg-zinc-800 text-zinc-300 border-zinc-700 hover:bg-zinc-700' : 'bg-white text-portal-text-main border-portal-border shadow-sm hover:bg-portal-surface-hover'}`}
            >
              <Globe size={14} className={isCleanMode ? 'text-cyan-400' : 'text-portal-brand'} />
              {showPerspectives ? 'Hide Perspectives' : 'Show Other Perspectives'}
            </button>
          </div>

          {showPerspectives && (
            <div className={`mb-8 p-5 rounded-xl border ${isCleanMode ? 'bg-zinc-900 border-zinc-800' : 'bg-portal-surface border-portal-border shadow-inner'}`}>
              <h4 className={`text-xs font-bold uppercase tracking-widest mb-4 font-mono ${isCleanMode ? 'text-zinc-400' : 'text-portal-text-muted'}`}>Global Reporting Perspectives</h4>
              <div className="space-y-3">
                <div className={`p-3 rounded-lg border ${isCleanMode ? 'bg-[#09090b] border-zinc-800' : 'bg-portal-bg border-portal-border'}`}>
                  ...
                  <p className={`text-xs ${isCleanMode ? 'text-zinc-400' : 'text-portal-text-muted'}`}>Emphasizes the privacy and regulatory implications of this event, rather than the technological breakthrough.</p>
                </div>
                <div className={`p-3 rounded-lg border ${isCleanMode ? 'bg-[#09090b] border-zinc-800' : 'bg-portal-bg border-portal-border'}`}>
                  <div className="flex justify-between items-center mb-1">
                  ...
                  <p className={`text-xs ${isCleanMode ? 'text-zinc-400' : 'text-portal-text-muted'}`}>Reports primarily on the supply chain disruptions and market opportunities created in the APAC region.</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className={`mt-12 p-6 sm:p-8 rounded-xl space-y-4 border ${isCleanMode ? 'bg-zinc-900 border-zinc-800' : 'bg-portal-surface border-portal-border'}`}>
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <button
                ...
                className={`px-4 py-2.5 rounded-lg text-xs font-semibold flex items-center justify-center space-x-2 transition-all font-mono group shrink-0 ${isCleanMode
