...
export default function NotificationCenter({ onNotificationClick, activeFeedCategories, theme = 'dark' }: NotificationCenterProps) {
  ...
  const iconBtnClass = theme === 'dark' 
    ? "text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors p-1.5 rounded" 
    : theme === 'sepia' 
      ? "text-[#726255] hover:text-[#2E241E] hover:bg-[#E6DEC9] transition-colors p-1.5 rounded" 
      : "text-neutral-500 hover:text-neutral-950 hover:bg-neutral-100 transition-colors p-1.5 rounded";

  const simulateBtnClass = theme === 'dark'
    ? "text-[11px] font-mono font-medium border border-zinc-800 bg-zinc-900 hover:bg-zinc-800 rounded px-2.5 py-1 text-zinc-300 hover:text-white flex items-center space-x-1"
    : theme === 'sepia'
      ? "text-[11px] font-mono font-medium border border-[#DFD5C1] bg-[#FAF6EE] hover:bg-[#EADFC9] rounded px-2.5 py-1 text-[#5C4D3E] hover:text-[#2E241E] flex items-center space-x-1"
      : "text-[11px] font-mono font-medium border border-neutral-200 bg-white hover:bg-neutral-50 rounded px-2.5 py-1 text-neutral-600 hover:text-neutral-900 flex items-center space-x-1 shadow-sm";
    ...
    <div id="newsletter-notification-hub" className="relative">
      ...
      <div className="flex items-center space-x-2 mr-3">
        ...
        >
          ...
          {unreadCount > 0 && (
            <span id="unread-pill-count" className="absolute -top-0.5 -right-0.5 bg-[#ef4444] text-white text-[9px] font-bold h-4 w-4 rounded-full flex items-center justify-center border-2 border-white scale-110">
              {unreadCount}
            </span>
          )}
        </button>
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className={`absolute right-0 mt-3 w-80 sm:w-96 border rounded-lg shadow-xl overflow-hidden z-50 font-sans ${
        ...
        <AnimatePresence>
          {toasts.map((toast) => (
            <motion.div
              ...
              className={`p-4 flex gap-3 pointer-events-auto cursor-pointer border rounded-lg shadow-2xl transition-all ${
