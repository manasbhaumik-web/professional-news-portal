            ...
            {isAdMinimized ? (
                ...
                <section id="top-headlines-carousel" className="mb-4 text-white border-0">
                    <div className="flex gap-4 items-stretch">
                        {/* News Headline Item */}
                        <div
                            className={`flex-1 flex cursor-pointer transition-opacity duration-300 ${isTransitioning ? 'opacity-0' : 'opacity-100'} relative group overflow-hidden shadow-2xl bg-gradient-to-br from-slate-900 via-slate-800 to-zinc-900 border border-slate-700/50 min-w-0`}
                            onClick={() => handleOpenArticle(article)}
                        >
                            {/* MINIMIZE BUTTON */}
                            <button onClick={(e) => { e.stopPropagation(); setIsAdMinimized(true); }} className="absolute top-3 right-3 z-20 p-1.5 bg-black/40 text-white hover:bg-black/70 transition-colors" title="Minimize">
                                <Minus size={14} />
                            </button>
                            ...
                            <button
                                ...
                                className="absolute left-3 top-1/2 -translate-y-1/2 z-20 p-2 bg-black/40 text-white hover:bg-black/70 transition-colors opacity-0 group-hover:opacity-100"
                            >
                                <ChevronLeft size={20} />
                            </button>
                            ...
                            <button
                                ...
                                className="absolute right-3 top-1/2 -translate-y-1/2 z-20 p-2 bg-black/40 text-white hover:bg-black/70 transition-colors opacity-0 group-hover:opacity-100"
                            >
                            ...
                            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex items-center justify-center gap-1.5 z-20">
                                {topHeadlines.map((_, idx) => (
                                    <button
                                        ...
                                        className={` transition-all duration-300 ${idx === activeIndex ? 'w-5 h-1 bg-portal-brand' : 'w-1.5 h-1 bg-white/40 hover:bg-white/70'}`}
