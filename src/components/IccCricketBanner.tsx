...
export default React.memo(function IccCricketBanner({
  ...
  return (
    <section id="icc-cricket-banner" className="mb-2 p-3 sm:p-4 border rounded-xl flex flex-col items-start gap-3 shadow-sm transition-colors bg-portal-surface border-blue-500/30 relative overflow-hidden">
      ...
      <div className="flex overflow-x-auto space-x-3 pb-1 w-full scrollbar-none relative z-10">
        {cricketLoading ? (
          ...
          cricketMatches.map((game, idx) => {
            ...
            return (
              <div key={idx} className="flex flex-col bg-portal-bg p-3.5 rounded-lg border border-portal-border shadow-sm min-w-[280px]">
                <div className="flex flex-col w-full">
                  <div className="flex items-center gap-1.5 mb-2 border-b border-portal-border/40 pb-1.5">
                    {game.matchType && <span className="text-[9px] font-bold text-portal-text-main bg-portal-surface px-1.5 py-0.5 rounded uppercase">{game.matchType}</span>}
