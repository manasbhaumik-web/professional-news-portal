  ...
  return (
    <section id="sidebar-advertisement" className="border rounded-xl overflow-hidden bg-portal-surface border-portal-border shadow-sm hover:border-portal-brand/40 transition-all cursor-pointer group relative">
      ...
      <div className="w-full h-28 bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <div className="text-white text-2xl font-black tracking-tight mb-0.5">NexAI Pro</div>
        <div className="text-white/80 text-[10px] font-mono uppercase tracking-widest">Intelligence Suite 2026</div>
    ...
    <section id="alerts-sidebar-card" className="border-2 rounded-xl flex flex-col overflow-hidden transition-all bg-portal-surface border-red-500/40 shadow-lg shadow-red-500/10 relative">
      ...
      <div className="p-4 space-y-4 relative z-10">
        {relatedArticles
          ...
          .map((art) => {
          return (
            <div key={art.id} className="cursor-pointer group p-2 -m-2 rounded-lg hover:bg-red-500/5 transition-colors border border-transparent hover:border-red-500/10" onClick={() => handleOpenArticle(art)}>
              <div className="inline-block align-top text-xs">
                <p className="font-semibold leading-snug text-portal-text-main group-hover:text-red-500 transition-colors line-clamp-2 inline">{art.title}</p>
                <span className="block text-[10px] font-mono text-portal-text-muted mt-1">{art.date}</span>
              </div>
            </div>
          );
        })}
      </div>
      <footer className="p-2 border-t text-[10px] font-mono text-center transition-colors border-red-500/20 bg-red-500/5 text-portal-text-muted relative z-10">
        Encrypted via Telecom Core
      </footer>
    </section>
  );
}
    ...
    <section id="trending-topics-card" className="border-2 rounded-xl p-5 transition-all bg-portal-surface border-portal-brand/30 shadow-lg shadow-portal-brand/10 relative overflow-hidden">
      ...
      <div className="space-y-3 relative z-10">
        {relatedArticles
          ...
          >
            <div className="text-[10px] font-mono text-portal-brand mb-0.5 font-bold tracking-widest">{idx + 1} • {art.category}</div>
