...
export default function Header({
  ...
  return (
    <header id="primary-editorial-nav" className="sticky top-0 z-40 h-16 flex items-center justify-between px-4 sm:px-8 border-b border-portal-border transition-colors shrink-0 backdrop-blur-md bg-opacity-95 bg-portal-surface text-portal-text-main shadow-sm">
      <div className="flex items-center space-x-4 sm:space-x-8">
        <div id="brand-logo" className="text-lg sm:text-xl font-bold tracking-tighter flex items-center cursor-pointer select-none transition-colors text-portal-text-main" onClick={() => setActiveTab('trending')}>
          <div className="w-8 h-8 rounded flex items-center justify-center mr-2 shadow-lg transition-all bg-portal-brand">
      ...
      <div className="flex items-center space-x-2 sm:space-x-4">
        <div id="theme-preset-selector" className="flex items-center p-1 rounded-lg border bg-portal-bg border-portal-border">
          {(['dark', 'light', 'sepia'] as const).map((t) => (
            <button
              key={t}
              id={`theme-btn-${t}`}
              onClick={() => setPortalTheme(t)}
              className={`p-1.5 rounded-md transition-all ${portalTheme === t
        ...
        <div id="usr-profile-chip" className="flex items-center space-x-2 border-l pl-4 sm:pl-6 select-none border-portal-border">
          <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-700 to-indigo-800 flex items-center justify-center text-xs font-bold text-white shadow-inner">
            AE
          </div>
