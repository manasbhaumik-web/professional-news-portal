...
export default function FifaWorldCupPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Hero Header */}
      <div className="relative rounded-2xl overflow-hidden border border-portal-border shadow-2xl bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f3460]">
        ...
        <div className="relative z-10 p-6 sm:p-10">
          <div className="flex items-center gap-2 text-[#c9a84c] font-mono text-xs uppercase tracking-widest font-bold mb-3">
            ...
            <span className="w-1.5 h-1.5 bg-red-500 rounded-full animate-ping ml-2" />
          ...
          <div className="flex flex-wrap gap-2 mt-5">
            {[
              ...
              <div key={host.label} className="flex items-center gap-2 bg-white/10 border border-white/20 backdrop-blur-sm rounded-full px-4 py-1.5 text-xs text-white font-semibold">
                <span>{host.flag}</span>
            ...
            <div className="flex items-center gap-2 bg-[#c9a84c]/20 border border-[#c9a84c]/40 backdrop-blur-sm rounded-full px-4 py-1.5 text-xs text-[#c9a84c] font-bold font-mono">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          disabled={isLoading}
          className="flex items-center gap-2 text-xs font-mono font-bold text-portal-text-muted hover:text-portal-text-main transition-colors border border-portal-border rounded-lg px-3 py-1.5 hover:bg-portal-surface disabled:opacity-50"
        >
      ...
      {/* Error Banner */}
      {error && (
        <div className="flex items-center gap-2 text-xs font-mono text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded-lg p-3">
      ...
      <div className="flex flex-wrap gap-2">
        {sources.map(src => (
          <button
            ...
            className={`px-3 py-1 rounded-full text-[11px] font-mono font-bold uppercase tracking-wider transition-all border ${
              activeFilter === src
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 9 }).map((_, i) => (
            <div key={i} className="rounded-xl border border-portal-border bg-portal-surface animate-pulse overflow-hidden">
              <div className="h-40 bg-portal-surface-hover" />
              <div className="p-4 space-y-3">
                <div className="h-3 bg-portal-surface-hover rounded w-1/3" />
                <div className="h-4 bg-portal-surface-hover rounded w-full" />
                <div className="h-4 bg-portal-surface-hover rounded w-4/5" />
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {filtered.length === 0 ? (
              <div className="col-span-full text-center py-16 text-portal-text-muted font-mono text-sm border border-dashed border-portal-border rounded-xl">
              ...
              filtered.map((article, idx) => (
                <motion.a
                  ...
                  className="group flex flex-col rounded-xl border border-portal-border bg-portal-surface hover:border-[#c9a84c]/50 hover:shadow-[0_4px_20px_rgba(201,168,76,0.1)] transition-all overflow-hidden cursor-pointer"
                >
                  {/* Image */}
                  <div className="relative h-44 overflow-hidden bg-portal-surface-hover shrink-0">
                    ...
                    {/* Source badge */}
                    <div className="absolute top-2 left-2 flex items-center gap-1.5 bg-black/60 backdrop-blur-sm text-white text-[10px] font-bold font-mono px-2 py-1 rounded-full border border-white/10">
                      <span>{article.sourceFlag}</span>
                      <span>{article.source}</span>
                    </div>
                    {/* External link icon */}
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-[#c9a84c] text-[#1a1a2e] p-1 rounded-full">
      ...
      <div className="rounded-xl border border-portal-border bg-portal-surface p-5">
        ...
        <div className="flex flex-wrap gap-3">
          {FIFA_FEEDS.map(feed => (
            <div key={feed.name} className={`flex items-center gap-2 text-[11px] font-mono px-3 py-1.5 rounded-full border ${
              loadedSources.includes(feed.name)
                ? 'border-green-500/30 bg-green-500/5 text-green-500'
                : 'border-portal-border text-portal-text-muted'
            }`}>
              <span>{feed.flag}</span>
              <span>{feed.name}</span>
              <span className={`w-1.5 h-1.5 rounded-full ${loadedSources.includes(feed.name) ? 'bg-green-500 animate-pulse' : 'bg-portal-text-muted/30'}`} />
            </div>
          ))}
        </div>
      </div>
