...
export default function FifaAllScoresPage({ onBack, onSelectMatch }: FifaAllScoresPageProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Page Header */}
      <div className="relative rounded-2xl overflow-hidden border border-[#c9a84c]/30 shadow-[0_4px_30px_rgba(201,168,76,0.12)] bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f3460]">
        ...
        <div className="relative z-10 p-5 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {apiStatus === 'live' && (
              <span className="flex items-center gap-1 text-[10px] font-mono text-green-400 bg-green-500/10 border border-green-500/20 px-2 py-1 rounded-full">
                <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span> Live API
              </span>
            )}
            {apiStatus === 'demo' && (
              <span className="flex items-center gap-1 text-[10px] font-mono text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2 py-1 rounded-full">
                ⚠ Demo Data
              </span>
            )}
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-mono font-bold rounded-lg bg-white/10 hover:bg-white/20 text-white border border-white/10 transition-all disabled:opacity-50"
            >
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0 scrollbar-none hide-scrollbar">
        {(['ALL', 'UPCOMING', 'LIVE', 'SCORERS'] as const).map(f => (
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-[11px] font-mono font-bold uppercase tracking-wider border transition-all shrink-0 ${filterStatus === f
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="animate-pulse rounded-xl border border-portal-border bg-portal-surface p-4 flex items-center gap-4">
              <div className="w-10 h-10 rounded-full bg-portal-surface-hover shrink-0" />
              <div className="flex-1 space-y-2">
                <div className="h-3 bg-portal-surface-hover rounded w-2/3" />
        ...
        <div className="text-center py-16 text-portal-text-muted font-mono text-sm border border-dashed border-portal-border rounded-xl">
          No matches found for this filter.
        </div>
      ) : filterStatus === 'SCORERS' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {scorers.map((s, idx) => (
             <div key={idx} className="flex items-center gap-4 p-4 rounded-xl border border-portal-border bg-portal-surface hover:border-[#c9a84c]/30 hover:shadow-[0_2px_16px_rgba(201,168,76,0.07)] transition-all">
               <div className="w-12 h-12 rounded-full overflow-hidden shrink-0 border border-portal-border relative bg-portal-surface-hover flex items-center justify-center font-bold text-lg text-portal-text-muted">
               ...
               <div className="flex flex-col flex-1 min-w-0">
                 ...
                 <div className="flex items-center gap-1.5 text-xs text-portal-text-muted mt-0.5">
                   {s.flag && <img src={s.flag.startsWith('http') ? s.flag : `https://flagcdn.com/${s.flag}.svg`} alt={s.team} className="w-4 h-4 rounded-full object-cover shrink-0" onError={e => { (e.target as HTMLImageElement).style.opacity = '0.3'; }} />}
            ...
            {filtered.map((m, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex flex-col p-3 rounded-xl border border-portal-border bg-portal-surface transition-all group"
                >
                  <div className="flex items-center gap-3 w-full">
                    ...
                    <div className="flex items-center gap-1.5 w-[90px] shrink-0 justify-end">
                      ...
                      <img src={img1} alt={m.team1}
                        className="w-7 h-7 rounded-full object-cover border border-portal-border/50 shrink-0"
                    ...
                    <div className="flex flex-col items-center flex-1 min-w-0">
                      ...
                      <div className={`text-[8px] font-bold font-mono uppercase tracking-widest px-2 py-0.5 rounded-full border mt-0.5 ${sc} ${isLive ? 'animate-pulse' : ''}`}>
                        {sl}
                      </div>
                    </div>
