...
function MatchTypeBadge({ type }: { type: string }) {
  ...
  return (
    <span className={`inline-flex items-center text-[9px] font-mono font-bold px-2 py-0.5 rounded border ${cls}`}>
      {type || 'MATCH'}
    </span>
  );
}
...
function GenderBadge({ gender }: { gender?: 'Men' | 'Women' }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[9px] font-mono font-bold px-2 py-0.5 rounded-full border ${
        isWomen
...
function SkeletonCard() {
  return (
    <div className="animate-pulse rounded-xl border border-portal-border bg-portal-surface p-5 space-y-3">
      <div className="flex gap-2 items-center">
        <div className="h-4 w-12 bg-portal-surface-hover rounded" />
        <div className="h-4 w-2/3 bg-portal-surface-hover rounded" />
      </div>
      <div className="h-5 bg-portal-surface-hover rounded w-3/4" />
      <div className="h-3 bg-portal-surface-hover rounded w-1/2" />
      <div className="h-px bg-portal-surface-hover rounded" />
      <div className="h-3 bg-portal-surface-hover rounded w-1/3" />
    </div>
  );
}
...
export default function CricketLivePage() {
  ...
  return (
    ...
    >
      {/* ── Page Header ────────────────────────────────────────────── */}
      <div className="relative rounded-2xl overflow-hidden border border-blue-500/30 shadow-[0_4px_30px_rgba(59,130,246,0.12)] bg-gradient-to-br from-[#0a1628] via-[#0d1f3c] to-[#0a2855]">
        ...
        <div className="relative z-10 p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="w-2 h-2 rounded-full bg-blue-400 shadow-[0_0_10px_rgba(59,130,246,0.9)] animate-pulse" />
          ...
          <div className="flex items-center gap-3">
            {isMock ? (
              <span className="flex items-center gap-1.5 text-[10px] font-mono text-amber-400 bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 rounded-full">
                <WifiOff size={10} /> Demo Data
              </span>
            ) : (
              <span className="flex items-center gap-1.5 text-[10px] font-mono text-green-400 bg-green-500/10 border border-green-500/20 px-3 py-1.5 rounded-full">
                <Wifi size={10} />
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-[10px] font-mono text-white/60 hover:text-blue-400 border border-white/10 hover:border-blue-500/30 px-3 py-1.5 rounded-full transition-all disabled:opacity-50"
            >
          ...
          {TABS.map(({ id, label, Icon, color }) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-mono font-semibold rounded-t-lg border-t border-l border-r transition-all ${
                  active
...
function EmptyState({ message }: { message: string }) {
  return (
    <div className="text-center py-20 border border-dashed border-portal-border rounded-xl text-portal-text-muted font-mono text-sm">
      {message}
    </div>
  );
}
...
function LiveCard({ match: m, idx }: { match: LiveMatch; idx: number }) {
  return (
    <motion.div
      ...
      className="flex flex-col p-5 rounded-xl border bg-portal-surface border-red-500/30 shadow-[0_0_14px_rgba(239,68,68,0.08)] hover:shadow-[0_2px_20px_rgba(239,68,68,0.15)] transition-all"
    >
      {/* Header */}
      <div className="flex items-start justify-between gap-2 mb-3">
        ...
        <span className="shrink-0 flex items-center gap-1 text-[9px] font-mono font-bold text-red-400 bg-red-500/10 border border-red-500/20 px-2 py-0.5 rounded-full">
...
function ResultCard({ match: m, idx }: { match: ResultMatch; idx: number }) {
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.25, delay: idx * 0.04 }}
      className="flex flex-col p-5 rounded-xl border bg-portal-surface border-portal-border hover:border-green-500/30 hover:shadow-[0_2px_20px_rgba(34,197,94,0.08)] transition-all"
    >
      {/* Header */}
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex flex-col gap-1.5 min-w-0">
        ...
        <span className="shrink-0 flex items-center gap-1 text-[9px] font-mono font-bold text-green-400 bg-green-500/10 border border-green-500/20 px-2 py-0.5 rounded-full">
          <CheckCircle2 size={9} />
          FT
        </span>
      </div>
...
function FixtureCard({ match: m, idx }: { match: FixtureMatch; idx: number }) {
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.25, delay: idx * 0.04 }}
      className="flex flex-col p-5 rounded-xl border bg-portal-surface border-portal-border hover:border-blue-500/30 hover:shadow-[0_2px_20px_rgba(59,130,246,0.08)] transition-all"
    >
      {/* Header */}
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex flex-col gap-1.5 min-w-0">
        ...
        <span className="shrink-0 flex items-center gap-1 text-[9px] font-mono font-bold text-blue-400 bg-blue-500/10 border border-blue-500/20 px-2 py-0.5 rounded-full">
          <CalendarDays size={9} />
          UPCOMING
        </span>
      </div>
      ...
      {/* Countdown strip */}
      <div className="flex items-center gap-3 mb-3 bg-blue-500/5 border border-blue-500/10 rounded-lg px-3 py-2">
